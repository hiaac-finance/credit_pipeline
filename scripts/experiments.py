...
def summarize_credit_experiment(args):
    ...
    for dataset in datasets:
        ...
        for fold in range(10):
            ...
            models_files_1 = glob(f"{path_1}/{dataset}/{fold}/*.pkl")
            models_files_1 = [file for file in models_files_1 if "study" not in file]
            models_files_2 = glob(f"{path_2}/{dataset}/{fold}/*.pkl")
            ...
            metrics_folds_val.append(
                evaluate.get_fairness_metrics(model_dict, X_val, Y_val, A_val)
            )
            metrics_folds_test.append(
                evaluate.get_fairness_metrics(model_dict, X_test, Y_test, A_test)
            )
        ...
        metrics_val = pd.concat(metrics_folds_val)
        metrics_cols = metrics_val.columns.tolist()
        metrics_cols.remove("model")
        metrics_val[metrics_cols] = metrics_val[metrics_cols].abs()
        metrics_val_mean = metrics_val.groupby("model").mean()
        metrics_val_std = metrics_val.groupby("model").std()
        for col in metrics_val_mean.columns:
            metrics_val_mean[col + "_"] = (
        ...
        metrics_test = pd.concat(metrics_folds_test)
        metrics_test[metrics_cols] = metrics_test[metrics_cols].abs()
        metrics_test_mean = metrics_test.groupby("model").mean()
        metrics_test_std = metrics_test.groupby("model").std()
        for col in metrics_test_mean.columns:
            metrics_test_mean[col + "_"] = (
