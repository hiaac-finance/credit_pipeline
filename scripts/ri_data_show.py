...
filepath_data = Path(os.path.join(ri_datasets_path,f'DATA-{main_seed}.joblib'))
filepath = Path(os.path.join(ri_datasets_path,f'models-{main_seed}.joblib'))
# filepath_metric = Path(os.path.join(ri_datasets_path,f'kickout-{main_seed}.joblib'))
    
data_dict = joblib.load(filepath_data)
hist_dict = joblib.load(filepath)
# hist_kick = joblib.load(filepath_metric)
...
for i in range(low_AR,high_AR):
    ...
    if i == low_AR:
        ...
        filepath = Path(os.path.join(ri_datasets_path,f'final/auc-{main_seed}.joblib'))
        filepath.parent.mkdir(parents=True, exist_ok=True)
        joblib.dump(df_auc, filepath)
    break

    filepath = Path(os.path.join(ri_datasets_path,f'final/kickout-{main_seed}-{low_AR}_{i}.joblib'))
    filepath.parent.mkdir(parents=True, exist_ok=True)
